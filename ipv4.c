...
int ipv4_send (ipv4_layer_t * layer, ipv4_addr_t dst, uint8_t protocol,unsigned char * payload, int payload_len) {
  printf("ipv4_send()--Has entrado en el ipv4_send()\n");
  ...
  {
    printf("Aqui solo como cliente\n");
    int arp = arp_resolve(layer->iface, dst, &macdst);
    
    //Sacamos la dirección MAC de destino
    if (arp < 0)
    {
          printf("ipv4_send()--MAC destino no ubicada\n");
          return -1;
    }
    printf("ipv4_send()--Vamos a enviar el datagrama IP por ETH\n");
...
int ipv4_recv(ipv4_layer_t * layer, uint8_t protocol,unsigned char* buffer, ipv4_addr_t sender, int buf_len,long int timeout) 
{
  ...
  char macPropia_srt[MAC_STR_LENGTH];
  eth_getaddr(layer->iface, macPropia);
  mac_addr_str(macPropia,macPropia_srt);
  printf("ipv4_recv()--ESTA MAC ES LA NUESTRA: %s\n",macPropia_srt);
  int isIP;
  int isProtocol;
  ...
  if (buf_len > payload_len) {
    buf_len = payload_len;
      ...
      printf("ipv4_recv()--Paquete recibido proto\n");
      memcpy(buffer, pkt_ip_recv.payload, buf_len);
      printf("ipv4_recv()--Valor retornado payload_len %d\n",payload_len);
      printf("ipv4_recv()--Número de bytes recibidos: %d\n", eth_len);
      print_pkt((unsigned char *) &pkt_ip_recv, buf_len+HEADER_LEN_IP, HEADER_LEN_IP);
